...
class CMDParser():
    ...
    def __init__(self):
        ...
        analysis_parser.add_argument('-n',choices=['full_network','subnetwork'],default='full_network',help='Network size')
...
def runAnalysis(args,output='write'):
    ...
    if args.n == 'full_network':
        driver,intermediates,responses = getNetworkGenes(args,verbose=True)
    elif args.n == 'subnetwork':
        driver,intermediates,responses = getSubnetworkGenes(args,verbose=False)
    else:
        logger.error('%s is not a supported network size.',args.n)
        sys.exit()
...
def writeWeights(args,driver,intermediates,responses,prioritization_parts,drug_target_max=5):
    ...
    if args.n == 'full_network':
        ...
        edge_dict = readSubnetworkEdges(args)
    else:
        logger.error('%s is not a supported network size.',args.n)
        sys.exit()

    #get gene pathway type
    v2pathType = getPathType(args,driver,intermediates,responses,edge_dict)
    v2DIIR = getPathTypeDIIR(args,driver,intermediates,responses,v2pathType,edge_dict)
...
def getPathTypeDIIR(args,driver,intermediates,responses,v2pathType,edge_dict):
    """
...
def getPathType(args,driver,intermediates,responses,edge_dict):
    """
...
def getNetworkInteractions(args):
    ...
    #check subnetwork option
    if args.n == 'subnetwork':
        #get filepath
        subnetwork_filepath = args.project_dir + '/subnetworkInteractions.txt'

        #whether the subnetwork interactions file exists, we want to run it
        #if it already exists, we want to check it

        #get subnetwork genes
        driver_sub,intermediates_sub,responses_sub = getSubnetworkGenes(args,verbose=True)
        allgenes_sub = list()
        allgenes_sub.append(driver_sub)
        allgenes_sub.extend(intermediates_sub)
        allgenes_sub.extend(responses_sub)
        allgenes_sub.sort()

        #get subnetwork interactions
        interactions_sub = getSubnetworkInteractions(args,allgenes_sub)

        #check if subnetwork interactions file exists
        if not os.path.isfile(subnetwork_filepath):
            #write subnetwork interactions to file
            writeSubnetworkInteractions(args,interactions_sub)
            logger.info('Wrote subnetwork interactions to %s',subnetwork_filepath)
        else:
            #read subnetwork interactions file
            logger.info('Found subnetwork interactions file %s',subnetwork_filepath)
            interactions_sub_read = readSubnetworkInteractions(args)

            #compare the two lists
            interactions_sub_sorted = sorted(interactions_sub)
            interactions_sub_read.sort()
            results = [a==b for a,b in zip(interactions_sub_read,interactions_sub_sorted)]
            if not all(results):
                logger.warning('Subnetwork interaction file %s contains error. Rewriting file.',subnetwork_filepath)
                #write subnetwork interactions to file
                writeSubnetworkInteractions(args,interactions_sub)

    return None

def writeSubnetworkInteractions(args,interactions_sub):
    """
    Writes subnetwork interaction file.
    Columns are a,b, and type
    a = first gene in interaction.
    b = second gene in interaction.
    type = PPI or TF.
    """
    filepath = args.project_dir + '/subnetworkInteractions.txt'

    fp = open(filepath, 'w')
    for interact in interactions_sub:
        (a, b, typ) = (interact[0], interact[1], interact[2])
        fp.write(a + '\t' + b + '\t' + typ + '\n')
    fp.close()
    return None

def readSubnetworkInteractions(args):
    """
    Reads subnetwork interaction file.
    Returns list of interactions with elements (a,b,type).
    a = first gene in interaction.
    b = second gene in interaction.
    type = PPI or TF.
    """
    ...
    if not os.path.isfile(filepath):
        logger.error('Subnetwork interactions file %s does not exist.',filepath)
        sys.exit()
    ...
    for line in rfp:
        ...
        if (len(toks) < 2) or (len(toks) >3):
            ...
            continue
        (a, b, typ) = (toks[0], toks[1], toks[2])
        ret[(a,b,typ)] = True
    rfp.close()
    ret = list(ret.keys())
    return(ret)

def getSubnetworkInteractions(args,genes):
    """
    Reads full network interactions.
    Returns subnetwork interactions as list with elements (a,b,type).
    a = first gene in interaction.
    b = second gene in interaction.
    type = PPI or TF.
    """

    #readfilepath for species
    if args.s == 'human':
        readfilepath = './databases/human_interactions.txt'
    elif args.s == 'mouse':
        readfilepath = './databases/mouse_interactions.txt'
    else:
        logger.error('%s is not a supported species.',args.s)
        sys.exit()

    #check readfilepath exists
    if not os.path.isfile(readfilepath):
        logger.error('Missing databases file %s. Run NetPert analysis.',readfilepath)
        sys.exit()

    ret = dict()

    rfp = open(readfilepath, 'r')
    for line in rfp:
        toks = line.strip().split()
        if (len(toks) < 2) or (len(toks) >3):
            if verbose:
...
def getSubnetworkGenes(args,verbose=False):
    """
    Reads user-defined driver and responses. Checks they are in the subnetwork.
    Returns driver, list of intermediates, and list of respones in the subnetwork.
    Subnetwork defined as driver, DIR, DIIR, and responses with edge from any of {driver,DIR,DIIR}.
    """
    if verbose:
        logger.info('Compiling subnetwork genes...')

    #whether the subnetwork files exist or not, we want to generate the list
    #if the file exists, we want to check it
    #get full network edges
    edge_dict = readEdges(args)
    edge_ab,edge_ba = edgeTuplesToDicts(edge_dict)

    #get full network genes
    driver,intermediates,responses = getNetworkGenes(args,verbose=False)
    ...
    for v in intermediates:
        typ = v2pathType[v]
        if typ == 'DIR':
            intermediates_sub[v] = True
        typ = v2DIIR[v]
        if typ:
            intermediates_sub[v] = True

    intermediates_sub = list(intermediates_sub.keys())
    intermediates_sub.sort()

    #responses
    for b in responses:
        if b in edge_ba.keys():
            for a in edge_ba[b]:
                if a in intermediates_sub or a in driver_sub:
                    responses_sub[b] = True
    responses_sub = list(responses_sub.keys())
    responses_sub.sort()

    #all genes in subnetwork
    allgenes_sub = list()
    allgenes_sub.append(driver_sub)
    allgenes_sub.extend(intermediates_sub)
    allgenes_sub.extend(responses_sub)
    ...
    #check if subnetwork genes filepath exists
    gene_filepath = args.project_dir + '/subnetworkGenes.txt'
    if not os.path.isfile(gene_filepath):
        #write subnetwork genes to file
        allgenes_sub_df = pd.DataFrame(allgenes_sub)
        allgenes_sub_df.to_csv(gene_filepath,sep='\t',index=False,header=False)
        logger.info('Wrote subnetwork gene list to %s',gene_filepath)
    else:
        #read subnetwork gene file
        allgenes_sub_read = readGenes(gene_filepath)
        if verbose:
            logger.info('Found subnetwork gene list file %s',gene_filepath)

        #compare the two lists
        results = [a==b for a,b in zip(allgenes_sub_read,allgenes_sub)]
        if not all(results):
            logger.warning('Subnetwork gene file %s contains error. Rewriting file.',gene_filepath)
            #write subnetwork genes to file
            allgenes_sub_df = pd.DataFrame(allgenes_sub)
            allgenes_sub_df.to_csv(gene_filepath,sep='\t',index=False,header=False)

    #check if subnetwork intermediates filepath exists
    intermediates_filepath = args.project_dir + '/subnetworkIntermediates.txt'
    if not os.path.isfile(intermediates_filepath):
        #write subnetwork intermediates to file
        intermediates_sub_df = pd.DataFrame(intermediates_sub)
        intermediates_sub_df.to_csv(intermediates_filepath,sep='\t',index=False,header=False)
        logger.info('Wrote subnetwork intermediate gene list to %s',intermediates_filepath)
    else:
        #read subnetwork intermediates file
        intermediates_sub_read = readGenes(intermediates_filepath)
        if verbose:
            logger.info('Found subnetwork intermediate gene list file %s',intermediates_filepath)

        #compare the two lists
        results = [a==b for a,b in zip(intermediates_sub_read,intermediates_sub)]
        if not all(results):
            logger.warning('Subnetwork intermediate gene file %s contains error. Rewriting file.',intermediates_filepath)
            #write subnetwork intermediates to file
            intermediates_sub_df = pd.DataFrame(intermediates_sub)
            intermediates_sub_df.to_csv(intermediates_filepath,sep='\t',index=False,header=False)

    #check if subnetwork responses filepath exists
    responses_filepath = args.project_dir + '/subnetworkResponses.txt'
    if not os.path.isfile(responses_filepath):
        #write subnetwork responses to file
        responses_sub_df = pd.DataFrame(responses_sub)
        responses_sub_df.to_csv(responses_filepath,sep='\t',index=False,header=False)
        logger.info('Wrote subnetwork response gene list to %s',responses_filepath)
    else:
        #read subnetwork responses file
        responses_sub_read = readGenes(responses_filepath)
        if verbose:
            logger.info('Found subnetwork response gene list file %s',responses_filepath)

        #compare the two lists
        results = [a==b for a,b in zip(responses_sub_read,responses_sub)]
        if not all(results):
            logger.warning('Subnetwork response gene file %s contains error. Rewriting file.',responses_filepath)
            #write subnetwork responses to file
            responses_sub_df = pd.DataFrame(responses_sub)
            responses_sub_df.to_csv(responses_filepath,sep='\t',index=False,header=False)

    #output subnetwork size
    if verbose:
        subnetwork_size = 1 + len(intermediates_sub) + len(responses_sub)
        logger.info('Subnetwork consists of %d genes: the driver %s, %d intermediates, and %d responses.',subnetwork_size,driver_sub,len(intermediates_sub),len(responses_sub))

    return(driver_sub,intermediates_sub,responses_sub)

def readSubnetworkResponses(args,driver_sub,allgenes_sub,verbose=False):
    """
    ...
    Checks if each response gene is in the subnetwork.
    Outputs list of network responses called subnetworkResponses.
    Returns a list of subnetwork responses.
    """

    #check if subnetwork response file exists
    subnetworkResponsesFilepath = args.project_dir + '/subnetworkResponses.txt'
    if os.path.isfile(subnetworkResponsesFilepath):
        if verbose:
            logger.info('Found subnetwork responses file %s. Delete this file if you want the subnetwork responses to be compiled again.',subnetworkResponsesFilepath)

        #check if file is empty
        if not os.stat(subnetworkResponsesFilepath).st_size:
            logger.error('The subnetwork responses file is empty. It must contain at least one gene symbol. Please delete this file and run NetPert again.')
            sys.exit()

        responses_sub = pd.read_csv(subnetworkResponsesFilepath,sep='\t',header=None)
        responses_sub = list(set(responses_sub[0].tolist()))
        responses_sub.sort()

        #remove driver_sub from subnetwork responses_sub
        if driver_sub in responses_sub:
            responses_sub.remove(driver_sub)
            if verbose:
                logger.warning('Found driver %s in the subnetwork response file. It was removed.' % (driver_sub))

        #check if responses_sub are in subnetwork and output list
        ret = list()
        for response_sub in responses_sub:
            if response_sub in allgenes_sub:
                ret.append(response_sub)
            else:
                logger.error('Not all subnetwork response genes in %s are in the subnetwork. Please delete this file and run NetPert analysis, again.', subnetworkResponsesFilepath)
                sys.exit()
        ...
        #check if responses are in subnetwork and output list
        ret = list()
        outfilepath = args.project_dir + '/subnetworkResponses.txt'
        fp = open(outfilepath, 'w')
        for response in responses:
            if response in allgenes_sub:
                ret.append(response)
                fp.write(response + '\n')
        fp.close()
        if verbose:
            logger.info('Found %s of %s response genes in the subnetwork. Wrote to %s.' % (str(len(ret)),str(len(responses)),outfilepath))
    return(ret)
...
def readSubnetworkEdges(args,verbose=False):
    """
    Reads interactions.
    Returns a dictionary of directed edges.
    Undirected edges are recorded as two directed edges in the dictionary.
    ...
    #check filepath exists
    if not os.path.isfile(filepath):
        logger.error('Missing subnetwork interaction file %s. Run NetPert analysis.',filepath)
        sys.exit()
